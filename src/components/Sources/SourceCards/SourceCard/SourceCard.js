...
const SourceCard = ({ source, subscribe, unsubscribe }) => {
	...
	return (
		<div className={[Style.Source, 'card'].join(' ')}>
			...
			{
				!isSubscribedTo() ?
					<button className="form-button" type="button" onClick={e => subscribe(source.id, e)}>Subscribe</button>
					:
					<button className="form-button active" type="button" onClick={e => unsubscribe(source.id, e)}>Unsubscribe</button>
			}
		</div>
	);
};
